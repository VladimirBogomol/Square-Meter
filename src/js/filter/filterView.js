const elements = {
  ...
  filterSubmit: document.querySelector(".filter__show"),
};
...
export function changeButtonText(number) {
  let btn = document.querySelector(".filter__show");

  // Отображаем доступные варианты на кнопке
  if (number !== 0) {
    btn.innerText = `Показать ${number} объектов`;
    btn.disabled = false;
  } else {
    btn.innerText = `Подходящие объекты не найдены`;
    btn.disabled = true;
  }  
};
...
export function getInput() {
  ...
  // 3. Получаем значения в input square и price
